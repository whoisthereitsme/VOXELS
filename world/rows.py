...
class ROWS:
    ...
    INTERNAL:
    - slot(row:Row) -> NDARR
    - set(row:Row, newrid:int=None, newmat:str=None) -> Row
    - reset(row:Row) -> None
    - remove(row:Row) -> None
    ...
    def slot(self, row: Row = None) -> NDARR:
        if row is None:
            raise ValueError("slot requires row")
        return self.array[int(row.mid)][int(row.rid)]

    def set(self, row: Row = None, newrid: int = None, newmat: str = None) -> Row:
        """
        Writes row.row into self.array at (row.mid,row.rid) after optional id updates.
        Returns a Row wrapper pointing at the stored slot (view).
        """
        if row is None:
        ...
        if newrid is not None:
            rid = int(newrid)
            row.rid = rid
            row.row[*ROW.IDS_RID] = np.uint64(rid)

        if newmat is not None:
            mid = int(self.mat.mid(name=newmat))
            row.mid = mid
            row.row[*ROW.IDS_MID] = np.uint64(mid)

        slot = self.array[mid][rid]
        slot[:] = row.row
        return Row(mid=mid, rid=rid, row=slot)

    def reset(self, row: Row = None) -> None:
        if row is None:
    ...
    def insert(
        ...
        raw = ROW.new(p0=p0, p1=p1, mat=mat, rid=rid, dirty=dirty, alive=alive)

        mid = int(ROW.MID(row=raw))
        rid = int(ROW.RID(row=raw))
        stored = Row(mid=mid, rid=rid, row=self.array[mid][rid])
        stored = self.set(row=stored)  # writes raw into slot
    ...
    def remove(self, row: Row = None) -> None:
        ...
        if rid != last:
            # moved is the last slot (copy its data, then overwrite rid slot)
            moved_slot = self.array[mid][last].copy()
            moved = Row(mid=mid, rid=last, row=self.array[mid][last])

            # remove moved's *old identity* from indices
            self.bvh.remove(row=moved)
            self.mdx.remove(row=moved)

            # overwrite rid slot with moved data, fixing rid inside payload
            moved_slot[*ROW.IDS_RID] = np.uint64(rid)
            self.array[mid][rid][:] = moved_slot

            # reinsert moved under new identity
    ...
    def search(self, pos: POS = None) -> Row:
        ...
        return self.bvh.search(pos=pos)  # BVH returns Row now

    # ============================================================
    # split (unchanged algorithmically; just Row objects at boundaries)
    # ============================================================
    ...
    def splitrow(self, pos: POS = None, p2: POS = None, mat: str = None) -> REQS:
        ...
        hit = self.search(pos=pos)                 # Row
        mat0 = self.mat.name(mid=int(hit.mid))     # old mat name
        hitrow = hit.row                           # NDARR view
        ...
        for i, (X0, X1) in enumerate(xs):
            for j, (Y0, Y1) in enumerate(ys):
                for k, (Z0, Z1) in enumerate(zs):
                    ...
                    newrow = self.insert(p0=(X0, Y0, Z0), p1=(X1, Y1, Z1), mat=use_mat)  # Row
    ...
    def split2(self, p0: POS = None, p1: POS = None, mat: str = None) -> REQS:
        ...
        hit = self.search(pos=p0)  # Row
        r0 = ROW.P0(row=hit.row)
        r1 = ROW.P1(row=hit.row)

        hitbox = intersect(a0=p0, a1=p1, b0=r0, b1=r1)
        if hitbox is None:
            return self.reqs(n=0)
    ...
    # ============================================================
    # merge (kept compatible with your current NDARR batch design)
    # NOTE: merge logic still uses mdx.search(mid,rid,axis) which returns Loc (mid,rid)
    # ============================================================
    ...
    def mergeax(self, mat: str = None, axis: int = None) -> REQS:
        ...
        while extra:
            ...
            row1 = Row(mid=mid, rid=prid, row=self.array[mid][prid])
